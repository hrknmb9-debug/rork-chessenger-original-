-- multiple_permissive_policies 対応（v2）: 明示的ポリシー名指定で確実に操作別に分割
-- cmd/pg_policies の挙動に依存せず、Linter 指摘のポリシーを直接指定
...
BEGIN
  ...
  LOOP
    ...
    IF pol_name LIKE '%読める%' THEN cmd_char := 'r';
    ELSIF pol_name LIKE '%削除できる%' OR pol_name LIKE '%離脱できる%' THEN cmd_char := 'd';
    ELSIF pol_name LIKE '%更新できる%' OR pol_name LIKE '%既読にできる%' THEN cmd_char := 'w';
